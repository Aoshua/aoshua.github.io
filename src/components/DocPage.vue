...
<template>
	<div class="relative min-h-screen overflow-x-clip bg-neutral-50 text-neutral-900">
		...
		<div class="relative mx-auto max-w-4xl px-4 sm:px-6">
			<header class="flex items-center justify-between gap-4 py-4 sm:py-8">
				...
				<RouterLink to="/" class="text-sm font-medium text-brand-600 transition hover:text-brand-600/70">&larr; Back</RouterLink>
			</header>

			<main
				class="doc-prose prose prose-neutral max-w-none rounded-2xl bg-white p-4 shadow-sm ring-1 ring-black/5 sm:p-10 prose-headings:font-display prose-headings:scroll-mt-8 prose-a:text-brand-600 prose-a:no-underline hover:prose-a:underline prose-blockquote:border-brand-600 prose-blockquote:not-italic prose-th:text-left"
				@click="onContentClick"
				v-html="html"
			></main>

			<footer class="mt-8 border-t border-neutral-200 py-6 text-center text-sm text-neutral-400 sm:mt-10 sm:py-8">
